...
// PUT /api/users/:id — ndrysho profilin (vetëm pronari)
router.put("/:id", verifyToken, async (req, res) => {
  try {
    ...
    const result = await pool.request()
      .input("id", sql.Int, id)
      .query("SELECT id, username, email, emri, mbiemri, datelindja, role FROM Users WHERE id = @id");
...
// PUT /api/users/:id/password — ndrysho fjalëkalimin (vetëm pronari)
router.put("/:id/password", verifyToken, async (req, res) => {
  try {
    ...
    if (!currentPassword || !newPassword)
      return res.status(400).json({ message: "Të dyja fushat janë të detyrueshme" });
    if (newPassword.length < 6)
      return res.status(400).json({ message: "Fjalëkalimi i ri duhet të ketë së paku 6 karaktere" });
...
// DELETE /api/users/:id — fshi llogarinë (vetëm pronari)
router.delete("/:id", verifyToken, async (req, res) => {
  try {
    ...
    // Anulimi i refresh tokenave
    await pool.request()
      .input("user_id", sql.Int, id)
      .query("UPDATE RefreshTokens SET is_revoked = 1 WHERE user_id = @user_id");

    // Anonimizo biletat (mos i fshi, ruaji për rekorde)
    await pool.request()
      .input("user_id", sql.Int, id)
