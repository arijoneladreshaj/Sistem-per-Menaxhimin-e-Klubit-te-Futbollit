...
router.get("/:id", verifyToken, requireRole(...ROLES), async (req, res) => {
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT t.*, p.emri + ' ' + p.mbiemri AS emri_lojtarit
        FROM Transfers t
        INNER JOIN Players p ON t.player_id = p.id
        WHERE t.id = @id
      `);
    if (!result.recordset[0]) return res.status(404).json({ error: "Transferimi nuk u gjet" });
...
router.post("/", verifyToken, requireRole(...ROLES), async (req, res) => {
  try {
    const { player_id, klubi_nisjes, klubi_destinacionit, shuma, data_transferimit, lloji, statusi, shenimet } = req.body;
    if (!player_id || !data_transferimit)
      return res.status(400).json({ error: "Lojtari dhe data janë të detyrueshme" });

    const pool = await poolPromise;
    await pool.request()
      .input("player_id",           sql.Int,           player_id)
      .input("klubi_nisjes",        sql.NVarChar(100),  klubi_nisjes        || null)
      .input("klubi_destinacionit", sql.NVarChar(100),  klubi_destinacionit || null)
      ...
      .query(`
        INSERT INTO Transfers (player_id, klubi_nisjes, klubi_destinacionit, shuma, data_transferimit, lloji, statusi, shenimet)
        VALUES (@player_id, @klubi_nisjes, @klubi_destinacionit, @shuma, @data_transferimit, @lloji, @statusi, @shenimet)
      `);
    ...
    await pool.request()
      ...
      .query(`
        UPDATE Transfers SET
          player_id           = @player_id,
          klubi_nisjes        = @klubi_nisjes,
          klubi_destinacionit = @klubi_destinacionit,
          shuma               = @shuma,
          data_transferimit   = @data_transferimit,
          lloji               = @lloji,
          statusi             = @statusi,
          shenimet            = @shenimet
        WHERE id = @id
