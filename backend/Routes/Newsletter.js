...
// POST /api/newsletter — regjistro email
...
// GET /api/newsletter — shiko të gjitha (admin)
const { verifyToken, requireAdmin } = require("../middleware/authMiddleware");
router.get("/", verifyToken, requireAdmin, async (req, res) => {
  try {
    await ensureTable();
    const pool = await poolPromise;
    const r = await pool.request().query("SELECT * FROM Newsletter ORDER BY created_at DESC");
