...
exports.create = async (req, res) => {
  try {
    ...
    if (!ALLOWED_ROLES.includes(role))
      return res
        .status(400)
        .json({ message: `Roli duhet të jetë: ${ALLOWED_ROLES.join(", ")}` });
    ...
    if (existing.recordset.length > 0) {
      for (const taken of existing.recordset) {
        if (taken.email.toLowerCase() === email.toLowerCase())
          return res
            .status(409)
            .json({ message: "Ky email është i regjistruar tashmë" });
    ...
    if (["Trajner", "Menaxher"].includes(role)) {
      await pool
        ...
        .query(
          "INSERT INTO Staff (club_id, user_id, emri, mbiemri, roli, statusi) VALUES (@club_id, @user_id, @emri, @mbiemri, @roli, @statusi)",
        );
    }

    if (role === "Lojtari") {
      const playerInsert = await pool
        .request()
        .input("club_id", sql.Int, 1)
        .input("emri", sql.NVarChar, emri.trim())
        .input("mbiemri", sql.NVarChar, mbiemri.trim())
        ...
        .input("statusi", sql.NVarChar, "Aktiv")
        .query(
          "INSERT INTO Players (club_id, emri, mbiemri, pozicioni, statusi) OUTPUT INSERTED.id VALUES (@club_id, @emri, @mbiemri, @pozicioni, @statusi)",
        );
...
exports.update = async (req, res) => {
  try {
    ...
    if (taken.recordset.length > 0)
      return res
        .status(409)
        .json({ message: "Username ose email është i zënë" });
    ...
    let query =
      "UPDATE Users SET username=@username, emri=@emri, mbiemri=@mbiemri, email=@email";
    if (password && password.length >= 6) {
      request.input("hash", sql.NVarChar, await bcrypt.hash(password, 10));
...
exports.changeRole = async (req, res) => {
  try {
    ...
    if (id === req.user.id)
      return res
        .status(400)
        .json({ message: "Nuk mund të ndryshosh rolin tënd" });
    ...
    if (roleId) {
      await pool
        ...
        .input("role_id", sql.Int, roleId)
        .query(
          "DELETE FROM UserRoles WHERE user_id = @user_id; INSERT INTO UserRoles (user_id, role_id) VALUES (@user_id, @role_id);",
        );
    }
...
exports.remove = async (req, res) => {
  try {
    ...
    if (id === req.user.id)
      return res
        .status(400)
        .json({ message: "Nuk mund të fshish llogarinë tënde këtu" });
