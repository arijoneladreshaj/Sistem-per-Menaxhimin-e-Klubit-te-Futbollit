...
exports.create = async (req, res) => {
  try {
    ...
    const orderRes = await pool
      ...
      .query(
        "INSERT INTO Orders (user_id,emri,mbiemri,email,telefoni,adresa,qyteti,shteti,subtotal,shipping,total) OUTPUT INSERTED.id VALUES (@user_id,@emri,@mbiemri,@email,@telefoni,@adresa,@qyteti,@shteti,@subtotal,@shipping,@total)",
      );
    ...
    for (const item of items) {
      await pool
        ...
        .query(
          "INSERT INTO OrderItems (order_id,product_id,emri,madhesia,sasia,cmimi) VALUES (@order_id,@product_id,@emri,@madhesia,@sasia,@cmimi)",
        );
    }
