...
exports.getAll = async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT p.*, c.emertimi AS club_name
      FROM Players p
...
exports.create = async (req, res) => {
  try {
    ...
    if (!emri || !mbiemri || !pozicioni)
      return res
        .status(400)
        .json({ error: "Emri, mbiemri dhe pozicioni janë të detyrueshme" });
    if (!POZICIONET_VALID.includes(pozicioni))
      return res
        .status(400)
        .json({
          error: `Pozicioni duhet të jetë: ${POZICIONET_VALID.join(", ")}`,
        });
    if (statusi && !STATUSET_VALID.includes(statusi))
      return res
        .status(400)
        .json({ error: `Statusi duhet të jetë: ${STATUSET_VALID.join(", ")}` });
    ...
    const result = await pool
      ...
      .input("vlera_tregut", sql.Decimal(15, 2), vlera_tregut || 0).query(`
        INSERT INTO Players (club_id, emri, mbiemri, data_lindjes, kombesia, pozicioni, numri_faneles, pesha, gjatesia, statusi, vlera_tregut)
        OUTPUT INSERTED.id
    ...
    res
      .status(201)
      .json({ success: true, message: "Lojtari u shtua", id: newId });

    try {
      const prefRes = await pool
        .request()
        .query(
          `SELECT user_id FROM UserPreferences WHERE topics LIKE '%lojtaret%'`,
        );
      ...
      console.error("[Notification lojtaret error]", notifErr.message);
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.update = async (req, res) => {
  try {
    ...
    await pool
      ...
      .input("rating", sql.Decimal(4, 1), rating ?? 70).query(`
        UPDATE Players SET
          emri=@emri, mbiemri=@mbiemri, data_lindjes=@data_lindjes, kombesia=@kombesia,
          pozicioni=@pozicioni, numri_faneles=@numri_faneles, pesha=@pesha, gjatesia=@gjatesia,
          statusi=@statusi, vlera_tregut=@vlera_tregut, apps=@apps, goals=@goals,
          assists=@assists, saves=@saves, clean_sheets=@clean_sheets, rating=@rating,
          updated_at=GETDATE()
        WHERE id=@id
      `);
...
exports.uploadPhoto = async (req, res) => {
  try {
    if (!req.file)
      return res.status(400).json({ error: "Nuk u dërgua asnjë file" });
...
exports.remove = async (req, res) => {
  try {
    ...
    await pool
      .request()
      .input("id", sql.Int, id)
      .query("DELETE FROM PlayerStats  WHERE player_id = @id");
    await pool
      .request()
      .input("id", sql.Int, id)
      .query("DELETE FROM MatchLineup  WHERE player_id = @id");
    await pool
      .request()
      .input("id", sql.Int, id)
      .query("DELETE FROM Injuries     WHERE player_id = @id");
    await pool
      .request()
      .input("id", sql.Int, id)
      .query("DELETE FROM Transfers    WHERE player_id = @id");
    await pool
      .request()
      .input("id", sql.Int, id)
      .query("DELETE FROM Contracts    WHERE player_id = @id");
    await pool
      .request()
      .input("id", sql.Int, id)
