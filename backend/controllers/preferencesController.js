...
exports.get = async (req, res) => {
  ...
  try {
    ...
    if (!result.recordset[0]) return res.json({ topics: [] });
    ...
    await pool
      ...
      .query(`
        IF EXISTS (SELECT 1 FROM UserPreferences WHERE user_id = @user_id)
          UPDATE UserPreferences SET topics = @topics WHERE user_id = @user_id
        ELSE
          INSERT INTO UserPreferences (user_id, topics) VALUES (@user_id, @topics)
      `);
