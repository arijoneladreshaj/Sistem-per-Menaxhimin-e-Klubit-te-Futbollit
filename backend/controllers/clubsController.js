...
exports.getAll = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT * FROM dbo.Clubs ORDER BY created_at DESC");
...
exports.getOne = async (req, res) => {
  try {
    ...
    if (!result.recordset[0])
      return res.status(404).json({ error: "Klubi nuk u gjet" });
...
exports.create = async (req, res) => {
  try {
    ...
    await pool
      ...
      .input("buxheti", sql.Decimal(15, 2), buxheti).query(`
    ...
    await pool
      ...
      .input("buxheti", sql.Decimal(15, 2), buxheti).query(`
        UPDATE dbo.Clubs SET
          emertimi=@emertimi, qyteti=@qyteti, stadiumi=@stadiumi,
          viti_themelimit=@viti_themelimit, logo=@logo,
          presidenti=@presidenti, buxheti=@buxheti, updated_at=GETDATE()
        WHERE id=@id
      `);
