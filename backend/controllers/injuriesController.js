...
async function updatePlayerStatus(pool, player_id) {
  const result = await pool
    ...
    .query(
      "SELECT COUNT(*) as cnt FROM Injuries WHERE player_id = @pid AND statusi IN ('Aktiv', 'Rikuperim')",
    );
...
exports.getAll = async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT i.*, p.emri + ' ' + p.mbiemri AS emri_lojtarit, p.pozicioni, p.numri_faneles
...
exports.getOne = async (req, res) => {
  try {
    ...
    if (!result.recordset[0])
      return res.status(404).json({ error: "Dëmtimi nuk u gjet" });
...
exports.create = async (req, res) => {
  try {
    ...
    await pool
      ...
      .query(
        "INSERT INTO Injuries (player_id,lloji_demtimit,pershkrimi,data_demtimit,data_rikthimit,statusi) VALUES (@player_id,@lloji_demtimit,@pershkrimi,@data_demtimit,@data_rikthimit,@statusi)",
      );
...
exports.update = async (req, res) => {
  try {
    ...
    await pool
      ...
      .query(
        "UPDATE Injuries SET player_id=@player_id,lloji_demtimit=@lloji_demtimit,pershkrimi=@pershkrimi,data_demtimit=@data_demtimit,data_rikthimit=@data_rikthimit,statusi=@statusi WHERE id=@id",
      );
...
exports.remove = async (req, res) => {
  try {
    ...
    if (player_id) await updatePlayerStatus(pool, player_id);
