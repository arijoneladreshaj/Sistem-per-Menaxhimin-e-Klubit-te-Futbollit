...
exports.getAll = async (req, res) => {
  try {
    ...
    const result = await pool.request().query(`
      SELECT c.*, p.emri + ' ' + p.mbiemri AS emri_lojtarit, p.pozicioni, p.numri_faneles
      FROM Contracts c INNER JOIN Players p ON c.player_id = p.id
...
exports.getOne = async (req, res) => {
  try {
    ...
    const result = await pool
      ...
      .query(
        "SELECT c.*, p.emri + ' ' + p.mbiemri AS emri_lojtarit FROM Contracts c INNER JOIN Players p ON c.player_id = p.id WHERE c.id = @id",
      );
  ...
  try {
    ...
    if (!player_id || !data_fillimit || !data_perfundimit)
      return res
        ...
        .json({
          error:
            "Player, data fillimit dhe data perfundimit janë të detyrueshme",
        });
    ...
    await pool
      ...
      .query(
        "INSERT INTO Contracts (player_id,club_id,data_fillimit,data_perfundimit,paga,bonus,lloji,statusi,kushtet) VALUES (@player_id,@club_id,@data_fillimit,@data_perfundimit,@paga,@bonus,@lloji,@statusi,@kushtet)",
      );
...
exports.update = async (req, res) => {
  try {
    ...
    await pool
      ...
      .input("kushtet", sql.NVarChar(500), kushtet || null)
      .query(
        "UPDATE Contracts SET player_id=@player_id,data_fillimit=@data_fillimit,data_perfundimit=@data_perfundimit,paga=@paga,bonus=@bonus,lloji=@lloji,statusi=@statusi,kushtet=@kushtet WHERE id=@id",
      );
