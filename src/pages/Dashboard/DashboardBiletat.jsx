...
export default function DashboardBiletat() {
  ...
  const [mapSector, setMapSector] = useState("lindje");
  ...
  const handleSave = async () => {
    ...
    if (seatNum < 1 || seatNum > maxSeats)
      return setMsg(
        `Ulësja duhet të jetë mes 1 dhe ${maxSeats} për sektorin "${form.sektori}"!`,
      );
  ...
  const handleStatusChange = async (id, statusi) => {
    try {
      ...
      setTickets((prev) =>
        prev.map((t) => (t.id === id ? { ...t, statusi } : t)),
      );
  ...
  const stats = {
    ...
    te_ardhura: tickets
      .filter((t) => t.statusi === "E shitur")
      .reduce((s, t) => s + Number(t.cmimi || 0), 0),
  };
  ...
  return (
    <div className="shell">
      ...
      <div className="main">
        <TopBar title="Menaxhimi i Biletave">
            ...
            {["tabela", "harta"].map((v) => (
              <button
                ...
                style={{
                  ...
                  background: view === v ? "#DA291C" : "#2a2a2a",
                  color: view === v ? "#fff" : "#888",
                }}
              >
                {v === "tabela" ? "Tabela" : "Harta"}
              </button>
            ))}
          </div>
        ...
        <div className="content">
          {msg && (
            <div
              className={`alert py-2 ${msg.includes("Gabim") ? "alert-danger" : "alert-success"}`}
            >
              {msg}
            </div>
          )}
          ...
          <div className="stats-row" style={{ marginBottom: 24 }}>
            {[
              ...
              {
                label: "Rezervuar",
                value: stats.rezervuar,
                sub: "Bileta të rezervuara",
              },
              {
                label: "Të ardhura",
                value: `€${stats.te_ardhura.toFixed(0)}`,
                sub: "Nga shitjet",
              },
            ].map((c) => (
          ...
          {/* HARTA */}
          {view === "harta" && (() => {
            const cfg = SECTOR_CONFIG[mapSector];
            const totalSeats = cfg.rows * cfg.cols;
            const seatMap = {};
            tickets
              .filter(t => (!mapMatch || String(t.match_id) === String(mapMatch)) && t.sektori === mapSector)
              .forEach(t => { seatMap[t.numri_uleses] = t; });

            const getSeatStyle = (seatNum) => {
              const t = seatMap[seatNum];
              const vip = isVipSeat(seatNum, mapSector);
              if (t?.statusi === "E shitur")    return { bg: "#7f1d1d", border: "#DA291C", color: "#fca5a5" };
              if (t?.statusi === "E rezervuar") return { bg: "#713f12", border: "#fbbf24", color: "#fde68a" };
              if (vip)                          return { bg: "#3b0764", border: "#7c3aed", color: "#c4b5fd" };
              return { bg: "#1e1e1e", border: "#2a2a2a", color: "#555" };
            };

            return (
              <div className="panel" style={{ marginBottom: 20 }}>
                <div className="panel-header" style={{ flexWrap: "wrap", gap: 10 }}>
                  <div className="panel-title">Harta e Ulëseve</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 4 }}>
                      {SEKTORET.map(s => (
                        <button
                          key={s}
                          onClick={() => setMapSector(s)}
                          style={{
                            padding: "4px 12px", borderRadius: 5, border: "none",
                            fontSize: 12, fontWeight: 600, cursor: "pointer",
                            background: mapSector === s ? "#DA291C" : "#2a2a2a",
                            color: mapSector === s ? "#fff" : "#888",
                          }}
                        >
                ...
                <div style={{ padding: "20px 24px" }}>
                  <div style={{ textAlign: "center", background: "#1a5c1a", color: "rgba(255,255,255,0.4)", fontSize: 10, letterSpacing: 3, padding: "6px 40px", borderRadius: 3, display: "inline-block", marginBottom: 16 }}>
                    ▬▬▬ FUSHA ▬▬▬
                  </div>
                  ...
                  <div
                    style={{
                      ...
                      gap: 5,
                      maxWidth: cfg.cols * 42,
                    }}
                  >
                    {Array.from({ length: totalSeats }, (_, i) => {
                      const seatNum = i + 1;
                      const s = getSeatStyle(seatNum);
                      const t = seatMap[seatNum];
                      return (
                        <div
                          key={seatNum}
                          title={t ? `Ulëse ${seatNum} — ${t.statusi}${t.emri_bleresit ? ` (${t.emri_bleresit} ${t.mbiemri_bleresit})` : ""}` : `Ulëse ${seatNum} — E lirë`}
                          onClick={() => t && openEdit(t)}
                          style={{
                            aspectRatio: "1",
                            borderRadius: 4,
                            background: s.bg,
                          ...
                          onMouseEnter={e => { if (t) e.currentTarget.style.transform = "scale(1.15)"; }}
                          onMouseLeave={e => { e.currentTarget.style.transform = "scale(1)"; }}
                        >
                          <span style={{ fontSize: 9, fontWeight: 700, color: s.color }}>{seatNum}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 20, flexWrap: "wrap", marginTop: 16 }}>
                    {[
                      { color: "#7f1d1d", border: "#DA291C", text: "E shitur" },
                      { color: "#713f12", border: "#fbbf24", text: "E rezervuar" },
                      { color: "#3b0764", border: "#7c3aed", text: "VIP" },
                      { color: "#1e1e1e", border: "#2a2a2a", text: "E lirë" },
                    ].map(l => (
                      <div key={l.text} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "#888" }}>
                        <div style={{ width: 12, height: 12, borderRadius: 3, background: l.color, border: `2px solid ${l.border}` }} />
                        {l.text}
                      </div>
                    ))}
                  </div>
          ...
          {/* FILTERS */}
          {view === "tabela" && <div className="panel">
            <div
              className="panel-header"
              style={{ padding: "12px 18px", gap: 10, flexWrap: "wrap" }}
            >
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {["Të gjitha", ...STATUSET].map((s) => (
                  ...
                  >
                    {s}{" "}
                    {s !== "Të gjitha" &&
                      `(${tickets.filter((t) => t.statusi === s).length})`}
                  </button>
                ))}
              </div>
              <input
                      ...
                      <tr key={t.id}>
                        ...
                        <td>
                          <select
                            ...
                            style={{
                              background:
                                STATUS_COLOR[t.statusi]?.bg || "#1a1a1a",
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    <input
                      name="mbiemri_bleresit"
                      value={form.mbiemri_bleresit}
                      onChange={handleChange}
                      style={inp}
                    />
                  </div>
                ...
                {(() => {
                  const sectorHasVip =
                    SECTOR_CONFIG[form.sektori]?.vipRows?.length > 0;
                  return (
                    <label
                      style={{
                        color: sectorHasVip ? "#aaa" : "#555",
                        fontSize: 13,
                        display: "flex",
                        alignItems: "center",
                        gap: 8,
                        cursor: sectorHasVip ? "pointer" : "not-allowed",
                      }}
                    >
                      <input
                        type="checkbox"
